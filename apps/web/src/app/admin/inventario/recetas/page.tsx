...
export default function InventarioRecetasPage() {
  ...
  async function crearSubreceta() {
    ...
    try {
      const res = await adminFetch('/inventario/subrecetas', {
        method: 'POST',
        body: JSON.stringify({
          nombre: nombreSub,
      ...
      if (!res.ok) {
        ...
        throw new Error(body?.message ?? 'No se pudo crear la subreceta.');
      }
      setNombreSub('');
      setRendimientoSub('');
      ...
      setErrorSub(err instanceof Error ? err.message : 'No se pudo crear la subreceta.');
    } finally {
  ...
  async function crearReceta() {
    ...
    try {
      const res = await adminFetch('/inventario/recetas', {
        method: 'POST',
        body: JSON.stringify({
          nombre: nombreReceta,
      ...
      setErrorReceta(err instanceof Error ? err.message : 'No se pudo crear la receta.');
    } finally {
        ...
        <>
          ...
          <section className="mt-10">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setMostrarFormReceta((v) => !v)}
            ...
            {mostrarFormReceta && (
              <div className="mt-3 space-y-2 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
                ...
                {errorReceta && <p className="text-xs text-red-600">{errorReceta}</p>}
                <button
                  type="button"
                  ...
                  className="rounded-md bg-brand-orange px-3 py-1.5 text-xs font-semibold text-white hover:opacity-90 disabled:opacity-50"
                >
                  {guardandoReceta ? 'Guardando…' : 'Crear receta'}
                </button>
              </div>
            )}

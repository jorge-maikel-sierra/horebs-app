...
const styles = StyleSheet.create({
  ...
  comprobanteEtiqueta: { color: '#FFD93D', fontSize: 9, fontWeight: 700, textTransform: 'uppercase' },
...
function ComprobanteDocumento({ pedido }: { pedido: PedidoAdminDto }) {
      ...
      <Page size="A5" style={styles.page}>
        ...
        <View style={styles.seccion}>
          ...
          <View style={styles.filaDato}>
            ...
            <Text style={styles.valor}>{MODALIDAD_LABEL[pedido.modalidad] ?? pedido.modalidad}</Text>
          </View>
...
function nombreArchivo(pedido: PedidoAdminDto): string {
  return `comprobante-${pedido.id.slice(0, 8)}.pdf`;
}

function datosCorreo(pedido: PedidoAdminDto): DatosCorreoFactura {
...
export class FacturaService {
  async generarPdf(pedido: PedidoAdminDto): Promise<Buffer> {
    return renderToBuffer(<ComprobanteDocumento pedido={pedido} />);
  }

  nombreArchivo(pedido: PedidoAdminDto): string {
    return nombreArchivo(pedido);
  }

  datosCorreo(pedido: PedidoAdminDto): DatosCorreoFactura {
    return datosCorreo(pedido);
  }
}
